function getValue() {
  /* function to get the value in the input*/
  ...
  changeThings(
    ...
    language,
    nbMonths
  ); /*Call the function who will do the main work here.*/
}

function createURLpage(name, language) {
  /*  Creer le lien pour la page wikipedia qu'on souhaite */
...
function findPage(usersC, id) {
  /*Do exaustif search to find the query matching the id i*/
...
function fillPage(usersC, language, L, n, D, j, nbMainCat) {
  /* Will display on screen the main information about the categories the users edit*/
  a = 0;

  for (i = 0; i < n; i++) {
    /*  In this example we sort by the number D[i][2]. As the list is assumed to be already sorted, we just go through it to display the first 10 */
...
function createTimestemps(date, D) {
  /* D est le nombre de mois */
...
function createLineinCategories(
  i,
  URLpage,
  DeltaEdit,
  CategoriesEdit,
  ...
  document.getElementById(istring).innerHTML +=
    "<div class='row'> <div class='col-2 bg-info  border' >" +
    URLpage +
    "</div> <div class='col-2 bg-info  border' >" +
    DaysEdit +
    "  </div> <div class='col-1 bg-info  border' > " +
    DeltaEdit +
    " </div><div class='col-1 bg-info  border' > " +
    Numberofview +
    " </div> <div class='col-6 bg-info  border' > <div class='row' id = " +
    ((line + 1) * 10 + i) +
    " hidden = true >" +
    CategoriesEdit +
    " </div><div class= 'row'><button type='button' class ='btn btn-success mb-2' onclick= " +
    '" displayValue(+' +
    ((line + 1) * 10 + i) +
    "," +
    10 +
...
function createBlock(n, D) {
  ...
  for (i = 0; i < n; i++) {
    if (i < n) {
      document.getElementById("titleInfo").innerHTML +=
        "<div class='col bg-info border' ><h3>" +
        D[i] +
        " </h3> <button type='button' class ='btn btn-success mb-2' onclick= " +
        '" displayValue(+' +
        i +
        "," +
        n +
        ');"' +
        ">Display main queries </button>  </div>";
      document.getElementById("infoQueries").innerHTML +=
...
function displayValue(i, n) {
  /*enables the users to one value or anothe*/
  ...
  if (n < 10) {
    for (j = 0; j < n; j++) {
      ...
      } else document.getElementById(j).hidden = true;
    }
  } else {
    if (document.getElementById(istring).hidden == false) {
      document.getElementById(istring).hidden = true;
