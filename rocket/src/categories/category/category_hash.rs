...
impl CategoryHash {
	...
	pub fn build_top_categories_intern<'a>(&'a self, bottoms: impl Iterator<Item = &'a Category>) -> Vec<&Category> {
		CategoryHash::graph_search_inter(
			&self.links,
			self.avg_degree,
			bottoms.map(|c| self.indices.get(&c.id).unwrap().to_owned()).collect(),
			HashSet::new(),
		).iter()
			.map(|&i| &self.vec[i])
			.collect()
