...
fn main() {
	...
	let text_links_2 = File::open(category_links_path).expect(&*format!(
		"{red}Something went wrong reading the category-links file : {}",
		category_links_path,
		red = color::Fg(color::Red)
	));
	...
	if pages {
		let page_links = run_pages(&categories, text_links_2);
		make_page_sql(&page_links, wp, output);

		println!(
			"{yellow}{bold}[FINISHED]{reset_c}{reset_s} ({} categories, {} links between categories and {} links between pages and categories)",
			categories.len(),
			category_links.len(),
			page_links.len(),
			bold = style::Bold,
